...
def quelleCol(N,joueur,grille,taille):
    while True:
        ...
        print("\n")
        if verification(grille,taille,colonne) == True:
            return colonne.upper()
        else:
...
def quelleCol_IA(N,grille,taille,joueur):
    ...
    for ordi in [joueur[0],inverse_joueur(joueur)]:
        for i in range(taille[0]):
            L = deepcopy(grille)
            if verification(grille,taille,Caract[i]) == True:
                joue([ordi,None],Caract[i],L,taille)
            vict,a = victoire(N, L, taille)
            if vict == True:
                return Caract[i]
...
def verification(grille,taille,colonne):
    Caract=list('ABCDEFGHIJKLMNOPQRSTUVWXYZ')
    Caract_taille =[]
    for i in range(taille[0]):
        Caract_taille.append(Caract[i])
    if colonne.upper() in Caract_taille :
...
if __name__ == "__main__":
    ...
    #test de repartition
    print("\n")
    print("Test de repartition avec nbr_IA = 1 \nrésultat attendu : [[1,'IA'],[2,'joueur']]")
    if repartition(1) == [[1,'IA'],[2,'joueur']]:
        print("OK")
    else:
        print("NOK")
